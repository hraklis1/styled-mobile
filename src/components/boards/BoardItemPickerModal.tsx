...
export function BoardItemPickerModal({
  board,
  visible,
  onClose,
}: {
  ...
  useEffect(() => {
    if (visible && board) {
      setSelected(new Set(board.itemIds ?? []));
      setQuery('');
      setCatFilter(null);
  ...
  const handleConfirm = () => {
    ...
    updateBoard.mutate({ id: board.id, itemIds }, { onSuccess: onClose });
  };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <View style={s.root}>
          <View style={s.header}>
            ...
            <Text style={s.headerTitle}>Add from Closet</Text>
            <TouchableOpacity onPress={handleConfirm} disabled={updateBoard.isPending} style={[s.headerSide, { alignItems: 'flex-end' }]}>
